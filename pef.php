...
<style>
  ...
  #pef-modal-close:hover {
    background: #8A1D19;
  }

  #pef-modal-inner-wrap {
    position: relative;
  }
...
<section class="pef-hero-bg relative pt-28 pb-24 px-4 overflow-hidden font-roboto">
  <div class="absolute top-0 left-0 w-80 h-80 rounded-full opacity-10 blur-3xl"
    style="background:#d32f2f;transform:translate(-40%,-40%)"></div>
  <div class="absolute bottom-0 right-0 w-96 h-96 rounded-full opacity-10 blur-3xl"
    style="background:#d32f2f;transform:translate(40%,40%)"></div>

  <div class="flex justify-center mb-6">
    <span
      class="pef-hero-kicker inline-flex items-center gap-2 bg-white bg-opacity-10 border border-white border-opacity-20 text-white px-4 py-1.5 rounded-full text-sm font-semibold uppercase tracking-widest">
  ...
  <div class="max-w-4xl mx-auto text-center">
    ...
    <p class="text-lg md:text-xl text-gray-300 leading-relaxed max-w-2xl mx-auto mb-8">
      Le <strong class="text-white">Programme Éducatif Fédéral</strong> de la FFF accompagne nos jeunes joueurs bien
      au-delà du terrain&nbsp;: citoyenneté, respect, valeurs et ouverture sur le monde.
    </p>
    <div
      class="inline-flex items-center gap-3 bg-white bg-opacity-10 backdrop-blur border border-white border-opacity-20 rounded-2xl px-6 py-4 text-white text-sm md:text-base font-semibold">
...
<section class="bg-gris py-20 px-4 font-roboto"
  style="background:linear-gradient(180deg, var(--pef-paper) 0%, #ffffff 100%)">
  <div class="max-w-6xl mx-auto">
    <div class="text-center mb-14 pef-appear">
      ...
      <p class="text-gray-500 mt-3 max-w-xl mx-auto text-sm md:text-base">Chaque pilier guide nos joueurs dans leur
        développement personnel et collectif.</p>
    </div>
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      foreach ($piliers as $i => [$nom, $icon, $bg, $col, $desc]):
        ?>
        <div
          class="pef-pilier-card bg-white rounded-2xl shadow-md border-2 border-transparent p-6 cursor-default pef-appear"
  ...
  <div class="max-w-3xl mx-auto flex flex-col sm:flex-row items-center gap-6 pef-appear">
    ...
    <div>
      ...
      <p class="text-gray-500 text-sm leading-relaxed">
        ...
        <a href="https://pef.fff.fr" target="_blank" rel="noopener noreferrer"
          class="text-rouge font-semibold hover:underline">
...
<!-- ═══ SECTION 4 — BLOG ════════════════════════════════════════════════════ -->
<section class="bg-gris py-20 px-4 font-roboto"
  style="background:linear-gradient(180deg, #ffffff 0%, var(--pef-paper) 100%)">
  <div class="max-w-6xl mx-auto">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        <?php foreach ($articles as $i => $a):
          ...
          <article
            ...
            style="animation-delay:<?= $i * 0.07 ?>s" data-id="<?= (int) $a['id'] ?>"
            onclick="pefOpenModal(<?= (int) $a['id'] ?>)" role="button" tabindex="0"
            aria-label="Lire l'article : <?= htmlspecialchars($a['titre']) ?>">
            <div class="relative h-48 overflow-hidden">
              <img src="<?= $img ?>" alt="<?= htmlspecialchars($a['titre']) ?>"
            ...
            <div class="p-5">
              ...
              <h3 class="font-bebas text-xl text-noir tracking-wide mb-2 leading-tight"><?= htmlspecialchars($a['titre']) ?>
              </h3>
...
<!-- ═══ MODALE ════════════════════════════════════════════════════════════════ -->
<div id="pef-modal" role="dialog" aria-modal="true" aria-labelledby="pef-modal-title"
  onclick="pefCloseOnOverlay(event)">
  <div id="pef-modal-inner-wrap">
    ...
    <div id="pef-modal-inner">
      ...
      <div id="pef-modal-body">
        <span id="pef-modal-badge"
          class="pef-cat-badge inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full mb-3"></span>
